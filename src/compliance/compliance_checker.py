...
class ComplianceChecker:
    ...
    def _load_standards(self) -> Dict:
        ...
        return {
            'SOC2': {
                ...
                'access_control': {
                    'required': ['authenticate', 'authorize', 'permission']
                },
                'audit_logging': {
                    'required': ['log', 'audit', 'track']
                }
            },
            'HIPAA': {
                'phi_handling': {
    ...
    def check_soc2_compliance(self, code: str, file_path: str) -> List[Dict]:
        ...
        for pattern in soc2_rules['data_encryption']['patterns']:
            if pattern in code.lower():
                has_encryption = any(req in code.lower() 
                                   for req in soc2_rules['data_encryption']['required'])
    ...
    def check_hipaa_compliance(self, code: str, file_path: str) -> List[Dict]:
        ...
        for forbidden in hipaa_rules['phi_handling']['forbidden']:
            if forbidden in code and any(p in code.lower() 
                                        for p in ['patient', 'medical', 'health']):
                violations.append({
                    'standard': 'HIPAA',
    ...
    def check_pci_dss_compliance(self, code: str, file_path: str) -> List[Dict]:
        ...
        for pattern in pci_rules['card_data']['patterns']:
            if pattern in code.lower():
                has_tokenization = any(req in code.lower() 
                                      for req in pci_rules['card_data']['required'])
    ...
    def check_gdpr_compliance(self, code: str, file_path: str) -> List[Dict]:
        ...
        for pattern in gdpr_rules['personal_data']['patterns']:
            if pattern in code.lower():
                has_protection = any(req in code.lower() 
                                    for req in gdpr_rules['personal_data']['required'])
    ...
    def run_all_compliance_checks(self, code: str, file_path: str, 
